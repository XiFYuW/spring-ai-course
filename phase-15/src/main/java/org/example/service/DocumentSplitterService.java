...
public class DocumentSplitterService {
    ...
    public List<Document> split(String text, String filename, SplitStrategy strategy, SplitOptions options) {
        ...
        List<Document> documents = switch (strategy) {
            ...
            case AGENTIC -> splitAgentic(text, filename, options);
            case CODE_AWARE -> splitCodeAware(text, filename, options);
            case HIERARCHICAL -> splitHierarchical(text, filename, options);
            case SLIDING_WINDOW -> splitSlidingWindow(text, filename, options);
            case STRUCTURED_DATA -> splitStructuredData(text, filename, options);
        };
    ...
    public List<Document> splitAgentic(String text, String filename, SplitOptions options) {
        ...
        // 使用启发式规则优化切割点
        int chunkIndex = 0;
        for (int i = 0; i < initialChunks.size(); i++) {
            TextChunk chunk = initialChunks.get(i);
            String content = chunk.getContent();

            // 尝试在主题边界优化切割
            String optimizedContent = optimizeChunkBoundary(content, chunkSize);

            Map<String, Object> metadata = buildMetadata(filename, chunkIndex, -1, "agentic");
            metadata.put("original_separator", chunk.getSeparator());
            metadata.put("optimization_applied", !content.equals(optimizedContent));
            metadata.put("chunk_group", i);

            documents.add(new Document(optimizedContent, metadata));
            chunkIndex++;
        }
     ...
     * 优化块边界 - 在主题边界处切割
     */
    private String optimizeChunkBoundary(String content, int targetSize) {
        ...
        int bestSplitPoint = targetSize;
        int minDistance = Integer.MAX_VALUE;

        for (String marker : topicMarkers) {
            int lastIndex = content.lastIndexOf(marker, targetSize);
            if (lastIndex > targetSize * 0.5) { // 至少保留 50% 内容
                int distance = Math.abs(targetSize - lastIndex);
                if (distance < minDistance) {
                    minDistance = distance;
                    bestSplitPoint = lastIndex + marker.length();
                }
            }
        }

        return content.substring(0, Math.min(bestSplitPoint, content.length()));
    }

    // ==================== 8. 代码感知切割 ====================

    /**
     * 代码感知切割 - 针对代码文件的特殊切割策略
     *
     * 特点：
     * 1. 按函数/类/方法边界切割
     * 2. 保留 import/依赖信息
    ...
    public List<Document> splitCodeAware(String text, String filename, SplitOptions options) {
        ...
        for (CodeBlock block : codeBlocks) {
            ...
            if (content.length() > maxChunkSize) {
                List<Document> subChunks = splitLargeCodeBlock(content, filename, block, imports, chunkIndex, maxChunkSize);
                documents.addAll(subChunks);
                chunkIndex += subChunks.size();
            } else {
                Map<String, Object> metadata = buildCodeMetadata(filename, chunkIndex, block, imports, language);
     ...
     * 解析代码块
     */
    private List<CodeBlock> parseCodeBlocks(String text, String language) {
        List<CodeBlock> blocks = new ArrayList<>();

        // 根据语言选择解析模式
        Pattern pattern = switch (language) {
            case "java", "csharp", "cpp", "kotlin" -> Pattern.compile(
            ...
            case "python" -> Pattern.compile(
                "(def|class)\\s+(\\w+)\\s*(?:\\([^)]*\\))?(?:\\s*->\\s*[^:]+)?:\\s*(?:\\'\\'\\'[\\s\\S]*?\\'\\'\\'|\\\"\\\"\\\"[\\s\\S]*?\\\"\\\"\\\"|[^\\n]*\\n(?:\\s+[^\\n]*\\n)*)",
                Pattern.MULTILINE
            );
            case "javascript", "typescript" -> Pattern.compile(
                "(?:(?:async\\s+)?function\\s*(?:\\*\\s*)?(\\w+)|(?:const|let|var)\\s+(\\w+)\\s*=\\s*(?:async\\s*)?(?:\\([^)]*\\)|[^=]*)\\s*=>|" +
                "(?:class)\\s+(\\w+)(?:\\s+extends\\s+\\w+)?\\s*\\{)",
                Pattern.MULTILINE
            );
            default -> Pattern.compile(
                "(?:function|def|func|fn|method|sub)\\s+(\\w+)",
                Pattern.MULTILINE
            );
        };
        ...
        while (matcher.find()) {
            // 添加块前的内容（可能是 import 或全局声明）
            if (matcher.start() > lastEnd) {
                String before = text.substring(lastEnd, matcher.start()).trim();
                if (!before.isEmpty() && !isOnlyImports(before, language)) {
                    blocks.add(new CodeBlock("preamble", before, "", 0));
                }
            }

            String blockType = detectBlockType(matcher.group(), language);
            String blockName = extractBlockName(matcher.group(), language);
            int startLine = countLines(text.substring(0, matcher.start()));

            // 提取完整的代码块（包括大括号内的内容）
            String fullBlock = extractFullBlock(text, matcher.start(), language);

            blocks.add(new CodeBlock(blockType, fullBlock, blockName, startLine));
            lastEnd = matcher.start() + fullBlock.length();
        }
        ...
        if (lastEnd < text.length()) {
            String remaining = text.substring(lastEnd).trim();
            if (!remaining.isEmpty()) {
                blocks.add(new CodeBlock("remaining", remaining, "", countLines(text.substring(0, lastEnd))));
            }
        }
     ...
     * 检测块类型
     */
    private String detectBlockType(String content, String language) {
        ...
        if (content.contains("def ") || content.contains("function ") || 
            content.matches(".*\\w+\\s*\\(.*\\)\\s*\\{.*")) return "method";
        return "block";
    }
     ...
     * 提取块名称
     */
    private String extractBlockName(String content, String language) {
        Pattern pattern = Pattern.compile("(?:class|interface|enum|def|function|func|fn)\\s+(\\w+)");
     ...
     * 提取完整的代码块
     */
    private String extractFullBlock(String text, int startPos, String language) {
        int braceCount = 0;
        int i = startPos;
        ...
        while (i < text.length()) {
            char c = text.charAt(i);
            char nextC = i + 1 < text.length() ? text.charAt(i + 1) : 0;
     ...
     * 提取 import 语句
     */
    private List<String> extractImports(String text, String language) {
        List<String> imports = new ArrayList<>();
        Pattern pattern = switch (language) {
            case "java", "kotlin" -> Pattern.compile("^import\\s+([^;]+);", Pattern.MULTILINE);
            case "python" -> Pattern.compile("^(?:from\\s+(\\S+)\\s+)?import\\s+([^\\n]+)", Pattern.MULTILINE);
            case "javascript", "typescript" -> Pattern.compile("^import\\s+.*?from\\s+['\"]([^'\"]+)['\"]|^require\\s*\\(\\s*['\"]([^'\"]+)['\"]\\s*\\)", Pattern.MULTILINE);
            default -> Pattern.compile("^import\\s+(.+)", Pattern.MULTILINE);
        };
     ...
     * 检查是否只有 import 语句
     */
    private boolean isOnlyImports(String text, String language) {
        String[] lines = text.split("\\n");
        for (String line : lines) {
            String trimmed = line.trim();
     ...
     * 分割大代码块
     */
    private List<Document> splitLargeCodeBlock(String content, String filename, CodeBlock block,
                                                List<String> imports, int startIndex, int maxSize) {
        List<Document> documents = new ArrayList<>();

        // 按逻辑段落分割（如方法内的逻辑块）
        String[] logicalBlocks = content.split("(?=\\n\\s*(?:if|for|while|switch|try|catch|else|elif))");

        StringBuilder currentChunk = new StringBuilder();
        int chunkIndex = startIndex;

        for (String logicalBlock : logicalBlocks) {
            if (currentChunk.length() + logicalBlock.length() > maxSize && currentChunk.length() > 0) {
                Map<String, Object> metadata = buildCodeMetadata(filename, chunkIndex, block, imports, detectLanguage(filename));
                metadata.put("partial_block", true);
                documents.add(new Document(currentChunk.toString().trim(), metadata));
                currentChunk = new StringBuilder();
                chunkIndex++;
            }
            currentChunk.append(logicalBlock);
        }
     ...
     * 构建代码元数据
     */
    private Map<String, Object> buildCodeMetadata(String filename, int chunkIndex, CodeBlock block,
                                                   List<String> imports, String language) {
        Map<String, Object> metadata = buildMetadata(filename, chunkIndex, -1, "code_aware");
        metadata.put("language", language);
        metadata.put("block_type", block.getType());
        metadata.put("block_name", block.getName());
        metadata.put("start_line", block.getStartLine());
        metadata.put("imports_count", imports.size());
        metadata.put("has_imports", !imports.isEmpty());
    ...
    private DocumentNode buildHierarchicalTree(String text, String filename) {
        ...
        for (int i = 0; i < chapters.length; i++) {
            ...
            DocumentNode chapterNode = new DocumentNode("chapter_" + i, "chapter", chapter, i, 1);

            // 第二层：按小节分割
            String[] sections = chapter.split("(?=(?:^|\\n)\\s*(?:\\d+\\.\\d+|\\d+\\.\\s+|【[^】]+】|\\[[^\\]]+\\]))");

            for (int j = 0; j < sections.length; j++) {
                String section = sections[j].trim();
                if (section.isEmpty()) continue;

                DocumentNode sectionNode = new DocumentNode("section_" + i + "_" + j, "section", section, j, 2);

                // 第三层：按段落分割
                String[] paragraphs = section.split("\n\s*\n");

                for (int k = 0; k < paragraphs.length; k++) {
                    String paragraph = paragraphs[k].trim();
                    if (paragraph.isEmpty()) continue;

                    DocumentNode paragraphNode = new DocumentNode(
                        "para_" + i + "_" + j + "_" + k, "paragraph", paragraph, k, 3
                    );
                    sectionNode.addChild(paragraphNode);
                }
     ...
     * 展平树为文档列表
     */
    private void flattenTree(DocumentNode node, List<Document> documents, String filename, int index, int maxSize) {
        ...
        if (content.length() > maxSize) {
            ...
            if (!node.getChildren().isEmpty()) {
                for (DocumentNode child : node.getChildren()) {
                    flattenTree(child, documents, filename, documents.size(), maxSize);
                }
                return;
            }

            // 没有子节点，需要强制分割
            List<String> chunks = forceSplitContent(content, maxSize);
            for (int i = 0; i < chunks.size(); i++) {
                Map<String, Object> metadata = buildHierarchicalMetadata(filename, documents.size(), node, i, chunks.size());
                documents.add(new Document(chunks.get(i), metadata));
            }
        } else {
            Map<String, Object> metadata = buildHierarchicalMetadata(filename, documents.size(), node, 0, 1);
            documents.add(new Document(content, metadata));
        }
    }
     ...
     * 强制分割内容
     */
    private List<String> forceSplitContent(String content, int maxSize) {
     ...
     * 查找句子结束位置
     */
    private int findSentenceEnd(String text, int targetPos) {
        String[] sentenceEndings = {". ", "? ", "! ", "。", "？", "！", "\n"};
        int bestPos = targetPos;

        for (String ending : sentenceEndings) {
            int pos = text.lastIndexOf(ending, targetPos);
            if (pos > targetPos - 100 && pos > 0) {
                bestPos = Math.max(bestPos, pos + ending.length());
            }
        }
     ...
     * 构建层次结构元数据
     */
    private Map<String, Object> buildHierarchicalMetadata(String filename, int chunkIndex,
                                                           DocumentNode node, int subIndex, int subTotal) {
        Map<String, Object> metadata = buildMetadata(filename, chunkIndex, -1, "hierarchical");
        metadata.put("node_id", node.getId());
        metadata.put("node_type", node.getType());
        metadata.put("hierarchy_level", node.getLevel());
        metadata.put("node_index", node.getIndex());
        metadata.put("sub_index", subIndex);
        metadata.put("sub_total", subTotal);
        metadata.put("has_children", !node.getChildren().isEmpty());
        metadata.put("children_count", node.getChildren().size());
    ...
    public List<Document> splitSlidingWindow(String text, String filename, SplitOptions options) {
        ...
        int overlapRatio = options.getChunkOverlap() != null ? options.getChunkOverlap() : 50; // 默认 50%
        ...
        while (startIndex < normalizedText.length()) {
            ...
            String chunk = normalizedText.substring(startIndex, endIndex).trim();

            if (!chunk.isEmpty()) {
                Map<String, Object> metadata = buildMetadata(filename, chunkIndex, -1, "sliding_window");
                metadata.put("window_start", startIndex);
                metadata.put("window_end", endIndex);
                metadata.put("window_size", endIndex - startIndex);
                metadata.put("stride", stride);
                metadata.put("overlap_ratio", overlapRatio);
                metadata.put("overlap_with_previous", chunkIndex > 0 ? Math.max(0, startIndex - (startIndex - stride)) : 0);
            ...
            // 防止无限循环
            if (startIndex >= normalizedText.length() || stride <= 0) {
                break;
            }
        }
    ...
    /**
     * 结构化数据切割 - CSV/JSON/XML 等
     *
     * 支持格式：CSV, JSON, XML, YAML
     * 保持记录完整性
     *
     * @param text 原始文本
     * @param filename 文件名
     ...
     * @return 切割后的文档列表
     */
    public List<Document> splitStructuredData(String text, String filename, SplitOptions options) {
        String format = detectStructuredFormat(filename, text);

        return switch (format) {
            case "csv" -> splitCsv(text, filename, options);
            case "json" -> splitJson(text, filename, options);
            case "xml" -> splitXml(text, filename, options);
            case "yaml", "yml" -> splitYaml(text, filename, options);
            default -> splitByCharacter(text, filename, options); // 回退到字符切割
        };
    }
    ...
    private List<Document> splitCsv(String text, String filename, SplitOptions options) {
        int maxRows = options.getChunkSize() != null ? options.getChunkSize() / 50 : 100; // 估算每行 50 字符

        List<Document> documents = new ArrayList<>();
        String[] lines = text.split("\\r?\\n");

        if (lines.length == 0) return documents;
        ...
        for (int i = 1; i < lines.length; i++) {
            ...
            if (rowCount >= maxRows && currentChunk.length() > header.length()) {
                Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "csv", headers, startRow, i - 1);
                documents.add(new Document(currentChunk.toString().trim(), metadata));
        ...
        if (currentChunk.length() > header.length() + 1) {
            Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "csv", headers, startRow, lines.length - 1);
            documents.add(new Document(currentChunk.toString().trim(), metadata));
        }
    ...
    private List<Document> splitJson(String text, String filename, SplitOptions options) {
        int maxSize = options.getChunkSize() != null ? options.getChunkSize() : DEFAULT_CHUNK_SIZE;
        ...
        for (int i = 1; i < jsonArray.length() - 1; i++) {
            ...
            if (!inString) {
                if (c == '{') braceCount++;
                    ...
                    braceCount--;
                    if (braceCount == 0) {
                        objects.add(jsonArray.substring(start, i + 1).trim());
                        start = i + 1;
                    }
                }
            }
     ...
     * 提取 JSON 条目
     */
    private List<String> extractJsonEntries(String jsonObject) {
        ...
        for (int i = 1; i < jsonObject.length() - 1; i++) {
            ...
            if (c == '"' && (i == 0 || jsonObject.charAt(i - 1) != '\\')) {
                inString = !inString;
            }
            ...
            if (!inString) {
                if (c == '{' || c == '[') braceCount++;
                    ...
                    entries.add("{" + jsonObject.substring(start, i).trim() + "}");
                    start = i + 1;
                }
            }
        }

        if (start < jsonObject.length() - 1) {
            entries.add("{" + jsonObject.substring(start, jsonObject.length() - 1).trim() + "}");
        }
     ...
     * 处理 JSON 对象
     */
    private void processJsonObjects(List<String> objects, String filename,
                                     List<Document> documents, int maxSize) {
        int chunkIndex = 0;
        StringBuilder currentChunk = new StringBuilder();
        currentChunk.append("[");
        int startObj = 0;
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if (currentChunk.length() + obj.length() > maxSize && currentChunk.length() > 1) {
                // 结束当前数组
                if (currentChunk.charAt(currentChunk.length() - 1) == ',') {
                    currentChunk.setLength(currentChunk.length() - 1);
                }
                currentChunk.append("]");

                Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "json", null, startObj, i - 1);
            ...
            currentChunk.append(obj).append(",");
        }
        ...
        if (currentChunk.length() > 1) {
            ...
            Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "json", null, startObj, objects.size() - 1);
            documents.add(new Document(currentChunk.toString(), metadata));
        }
    }
    ...
    private List<Document> splitXml(String text, String filename, SplitOptions options) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (currentChunk.length() + elem.length() > maxSize && currentChunk.length() > declaration.length()) {
                Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "xml", null, startElem, i - 1);
                documents.add(new Document(currentChunk.toString().trim(), metadata));
            ...
            currentChunk.append(elem).append("\n");
        }

        // 保存最后一个块
        if (currentChunk.length() > declaration.length()) {
            Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "xml", null, startElem, elements.size() - 1);
            documents.add(new Document(currentChunk.toString().trim(), metadata));
        }
     ...
     * 提取 XML 元素
     */
    private List<String> extractXmlElements(String text) {
     ...
     * 切割 YAML 文件
     */
    private List<Document> splitYaml(String text, String filename, SplitOptions options) {
        int maxSize = options.getChunkSize() != null ? options.getChunkSize() : DEFAULT_CHUNK_SIZE;
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            // 检测顶层键（无缩进或特定标记）
            if (line.matches("^[\\w-]+:") || line.matches("^---\\s*$") || line.matches("^#")) {
                if (currentChunk.length() > 0 && currentChunk.length() > maxSize) {
                    Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "yaml", null, startLine, i - 1);
                    documents.add(new Document(currentChunk.toString().trim(), metadata));
        ...
        // 保存最后一个块
        if (currentChunk.length() > 0) {
            Map<String, Object> metadata = buildStructuredMetadata(filename, chunkIndex, "yaml", null, startLine, lines.length - 1);
            documents.add(new Document(currentChunk.toString().trim(), metadata));
        }
     ...
     * 构建结构化数据元数据
     */
    private Map<String, Object> buildStructuredMetadata(String filename, int chunkIndex,
                                                         String format, List<String> headers,
                                                         int startIndex, int endIndex) {
        Map<String, Object> metadata = buildMetadata(filename, chunkIndex, -1, "structured_data");
        metadata.put("format", format);
        metadata.put("start_index", startIndex);
        metadata.put("end_index", endIndex);
        metadata.put("record_count", endIndex - startIndex + 1);

        if (headers != null && !headers.isEmpty()) {
            metadata.put("headers", headers);
            metadata.put("header_count", headers.size());
        }
    ...
    private static class CodeBlock {
        ...
        public CodeBlock(String type, String content, String name, int startLine) {
            this.type = type;
            this.content = content;
            this.name = name;
    ...
    private static class DocumentNode {
        ...
        public DocumentNode(String id, String type, String content, int index, int level) {
            this.id = id;
            this.type = type;
            this.content = content;
            this.index = index;
