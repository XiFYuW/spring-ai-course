...
import org.example.service.DocumentSplitterService;
...
public class VectorStoreController {
    ...
    public VectorStoreController(VectorStoreService vectorStoreService, FileProcessingService fileProcessingService) {
     ...
     * 支持参数：
     * - file: TXT 文件（必需）
     * - splitStrategy: 切割策略，可选
     *   - RECURSIVE: 递归字符切割（默认）
     *   - MARKDOWN: Markdown 结构切割
     *   - TOKEN: Token 感知切割
     *   - SEMANTIC: 语义切割（基于 Embedding 相似度）
     *   - SMART_PARAGRAPH: 智能段落切割
     *   - CHARACTER: 固定字符切割
     *   - AGENTIC: Agentic 切割（智能主题边界）
     *   - CODE_AWARE: 代码感知切割（支持 Java/Python/JS 等）
     *   - HIERARCHICAL: 层次结构切割（文档树）
     *   - SLIDING_WINDOW: 滑动窗口切割（高重叠率，适合 RAG）
     *   - STRUCTURED_DATA: 结构化数据切割（CSV/JSON/XML/YAML）
    ...
    public Mono<ResponseEntity<ApiResponse<FileUploadResponse>>> uploadFile(
            ...
            @RequestParam(name = "splitStrategy", defaultValue = "RECURSIVE") DocumentSplitterService.SplitStrategy splitStrategy,
        ...
        logger.info("Uploading file: {}, splitStrategy: {}, chunkSize: {}, overlap: {}",
        ...
        return fileProcessingService.processAndStore(filePart, splitStrategy, chunkSize, chunkOverlap)
